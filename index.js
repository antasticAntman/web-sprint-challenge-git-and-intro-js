...
function listOfNames(array) {
  let namesList = array.slice();
  let theName = [];
  for(let i = 0; i < array.length; i ++ ) {
    if (array[i].name === namesList[i].name){
      theName.push(array[i].name);
    }
  }return theName
}
...
function addArtist(array, name, years, genre,nationality, bio) {
  const duplicate = array.slice();
  duplicate.push({name, years, genre, nationality, bio})
  return duplicate
}
console.log(artists,'John Doe', '1988-2022', 'Full Stack Development', 'African American','I have a background in customer service at Big Retail Chain. I am attending BloomTech to become a Frontend Developer.')
...
1. Receive the artist array as an argument passed from the FIRST parameterX
2. Receive a string (nationality) as an argument passed from the SECOND parameterX
